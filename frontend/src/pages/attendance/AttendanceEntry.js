...
export default function AttendanceEntry() {
  ...
  const [manAction, setManAction]     = useState(null);    // "clock_in" | "clock_out"
  const [manPhoto, setManPhoto]       = useState(null);

  const manVideoRef                     = useRef(null);
  const manCanvasRef                    = useRef(null);
  const manStreamRef                    = useRef(null);
  ...
  async function openManCamera(action) {
    setManPhoto(null);
    setManAction(action);
    setAlert({ type: "", message: "" });
    try {
