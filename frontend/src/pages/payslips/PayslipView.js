            ...
            <table className="table table-bordered">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="text-end">{parseFloat(payslip.total_hours).toFixed(2)} hrs</td>
                </tr>
                <tr>
                  <td>Hourly Rate</td>
                  <td className="text-end">₹{parseFloat(payslip.hourly_rate).toFixed(2)} / hr</td>
                </tr>
                <tr>
                  <td>Daily Rate</td>
                  <td className="text-end">₹{parseFloat(payslip.daily_rate).toFixed(2)} / day</td>
                </tr>
                <tr className="fw-semibold">
                  <td>Gross Pay <span className="text-muted fw-normal small">(Days Worked × Daily Rate)</span></td>
                  <td className="text-end">₹{parseFloat(payslip.gross_pay).toFixed(2)}</td>
                </tr>
              </tbody>
              ...
              <tbody>
                ...
                <tr className="text-danger">
                  <td>ESI (0.75%)</td>
                  <td className="text-end">- ₹{parseFloat(payslip.esi).toFixed(2)}</td>
                </tr>
                <tr className="text-danger">
                  <td>PF (12%)</td>
                  <td className="text-end">- ₹{parseFloat(payslip.pf).toFixed(2)}</td>
                </tr>
              </tbody>
              ...
              <tfoot>
                <tr className="table-success fw-bold fs-5">
                  <td>Net Pay</td>
                  <td className="text-end">₹{parseFloat(payslip.net_pay).toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>

            <p className="text-muted small text-center mt-3">
              Gross Pay = {payslip.days_worked} days × ₹{parseFloat(payslip.daily_rate).toFixed(2)} = ₹{parseFloat(payslip.gross_pay).toFixed(2)}
              &nbsp;|&nbsp;
              Net Pay = ₹{parseFloat(payslip.gross_pay).toFixed(2)} − ₹{parseFloat(payslip.esi).toFixed(2)} − ₹{parseFloat(payslip.pf).toFixed(2)} = ₹{parseFloat(payslip.net_pay).toFixed(2)}
            </p>
          </div>
        </div>
